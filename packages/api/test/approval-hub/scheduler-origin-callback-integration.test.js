...
const load = (path) => import(`../../dist/${path}.js`);
const socket = () => ({ emitToUser() {}, broadcastToRoom() {}, broadcastAgentMessage() {} });
 ...
 */
async function appendSchedulerWakeRow(messageStore, threadId) {
 ...
 */
const schedulerWokenAuth = (registry, threadId, wakeMessageId) =>
  registry.create(OWNER, CAT, threadId, undefined, undefined, undefined, wakeMessageId);

const post = (app, url, auth, payload) =>
  app.inject({
    method: 'POST',
    url,
...
async function assertAnchoredToWakeRow(store, proposalId, threadId, wakeRow) {
